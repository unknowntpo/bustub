...
class ListNode {
 public:
  explicit ListNode(size_t frame_id, bool is_evictable, size_t k);
...
class MapNode {
 ...
 public:
  frame_id_t fid_;
  std::list<ListNode>::iterator it_;
  bool is_evictable_{false};
  size_t k_;
...
class LRUKReplacer {
 ...
 private:
  ...
  // evictable size
  size_t curr_size_{0};
  std::unordered_map<frame_id_t, std::list<ListNode>::iterator> node_store_;
